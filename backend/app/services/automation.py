...
def _extract_requested_contact_name(transcript: str) -> str:
    ...
    def clean(candidate: str) -> str:
        ...
        candidate = re.split(r"[?!.;,]|\b(?:can you|could you|would you)\b", candidate, maxsplit=1, flags=re.IGNORECASE)[0]
        candidate = candidate.strip(" .!?")
        candidate = re.sub(r"^(my|the)\s+", "", candidate, flags=re.IGNORECASE)
        candidate = re.sub(r"^(audio|video|facetime|phone|mobile|call)\s+", "", candidate, flags=re.IGNORECASE)
    ...
    call_match = re.search(
        r"\b(?:facetime|phone|call|video\s+call|audio\s+call)\b[,\s]*(.+)$",
        text,
        flags=re.IGNORECASE,
    )
    if call_match:
        return clean(call_match.group(1))
...
def _script_call(contact_name: str, contact_phone: Optional[str], *, video: bool) -> str:
    """Simulang script: prepare a phone or FaceTime call via the native app."""
    ...
    contact_label_re = json.dumps(re.escape(contact_name.split()[0] if contact_name else recipient))
    return f"""\
// Simulang script: Prepare {call_kind} with {contact_name}
// Generated by Tendly automation service
{_simulang_imports()}
{_simulang_helpers()}

try {{
  console.log('[Tendly] Preparing ' + {safe_kind} + ' with ' + {safe_contact_name})
  ...
  const newFaceTimeButton = await withSnapshot(
    tree,
    (nodes) => findNode(nodes, [AriaRole.Button], /new facetime/i),
    'New FaceTime button',
    2000,
  ).catch(() => null)
  if (newFaceTimeButton?.refId != null) {{
    tree.activate(newFaceTimeButton.refId)
    await sleep(800)
  }}

  const contactPattern = new RegExp({contact_label_re}, 'i')
  const suggestedContact = await withSnapshot(
    tree,
    (nodes) => findNode(
      nodes,
      [AriaRole.Button, AriaRole.Group, AriaRole.ListItem, AriaRole.Option, AriaRole.Row, AriaRole.Suggestion],
      contactPattern,
    ),
    'FaceTime suggested contact',
    2500,
  ).catch(() => null)

  if (suggestedContact?.refId != null) {{
    tree.activate(suggestedContact.refId)
  }} else {{
    const nodes = pageNodes(tree.snapshot(false))
    const recipientField = findNode(nodes, [AriaRole.Textbox, AriaRole.Searchbox, AriaRole.Combobox], /name|email|phone|search|to/i)
      ?? nodes.find((n) =>
        (n.role === AriaRole.Textbox || n.role === AriaRole.Searchbox || n.role === AriaRole.Combobox) && n.refId != null
      )

    if (recipientField?.refId != null) {{
  ...
  await withSnapshot(
    tree,
    (nodes) => findNode(nodes, [AriaRole.Button], /{call_label_re}/i),
    'FaceTime call button',
    5000,
  )
  console.log('[Tendly] Done: FaceTime is ready to call ' + {safe_contact_name} + ' with one button press')
}} catch (error) {{
  fail(error)
}}
"""
